...
export function FranqueadosContent() {
  ...
  // Local state for franqueados to update UI immediately
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/franqueados", {
        ...
        body: JSON.stringify({
          ...formData,
          masterId: user.id, // Assuming the logged-in user is the master
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || "Erro ao cadastrar franqueado");
      }

      // Add to local list optimistically or use returned data
      await fetchFranqueados();

      if (data.generatedPassword) {
        setGeneratedPassword(data.generatedPassword);
      } else {
        toast.success("Franqueado cadastrado e email enviado com sucesso!");
      }
      ...
      console.error("Error registering franqueado:", error);
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/users/password", {
        ...
        body: JSON.stringify({
          userId: selectedFranqueado?.id,
          email: selectedFranqueado?.email,
  ...
  const handleToggleStatus = (franqueado: Franqueado) => {
    setUserToToggleStatus(franqueado);
  };
  ...
  const confirmStatusChange = async () => {
    ...
    // const actionText = newStatus === "ACTIVE" ? "ativar" : "desativar"; // Not used here anymore
  ...
  return (
    <PageLayout
      title="Franqueados"
      subtitle="Gerencie os franqueados da sua franquia"
    >
      <div className={cn(isMaster && "dark")}>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          <StatCard
            title="Total"
            value={franqueados.length}
          ...
          <StatCard
            title="Ativos"
            value={franqueados.filter((f) => f.status === "active").length}
          ...
          <StatCard
            ...
            value={formatCurrency(
              franqueados.reduce((acc, f) => acc + (f.loanAmount ?? 0), 0),
            )}
          ...
          <StatCard
            title="Prefeituras"
            value={franqueados.reduce(
              (acc, f) => acc + (f.citiesRegistered ?? 0),
              0,
            )}
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            <Input
              ...
              className={cn("w-full pl-10", styles.input)}
            />
          ...
          <Button
            ...
            className={cn("flex items-center gap-2", styles.buttonPrimary)}
          >
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className={styles.header}>
                <tr>
                  <th
                    className={`px-6 py-3 font-medium uppercase ${styles.headerText}`}
                  >
                    Nome
                  </th>
                  <th
                ...
                {filteredFranqueados.map((f) => (
                  <tr key={f.id} className={`transition-colors ${styles.row}`}>
                    ...
                    <td className={`px-6 py-4 font-semibold ${styles.text}`}>
                      {formatCurrency(f.loanAmount ?? 0)}
                    </td>
                    <td className={`px-6 py-4 font-semibold ${styles.text}`}>
                      <div className="flex flex-col">
                        <span>
                          {formatCurrency((f.loanAmount ?? 0) * (f.commissionRate / 100))}
                        </span>
                    ...
                    <td className="px-6 py-4">
                      <StatusBadge
                        ...
                        variant={isMaster ? "modern" : "default"}
                      />
                    </td>
                    <td className="px-6 py-4">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => {
                              ...
                              setFormData({
                                ...
                                phone: f.phone,
                                document: f.document || "",
                                commissionRate: f.commissionRate.toString(),
                                address: f.address || "",
                                city: f.city || "",
                                state: f.state || "",
                                zipCode: f.zipCode || "",
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
                            className={f.status === "active" ? "text-red-600" : "text-green-600"}
        ...
        <Dialog open={!!generatedPassword} onOpenChange={() => setGeneratedPassword(null)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Como a opção de envio de email não foi selecionada, copie a senha abaixo e envie para o franqueado manualmente.
              </DialogDescription>
            </DialogHeader>
        ...
        <ConfirmDialog
          ...
          description={
            userToToggleStatus?.status === "active"
              ? `Tem certeza que deseja desativar o acesso de ${userToToggleStatus?.name}? O usuário não poderá mais acessar o sistema.`
              : `Tem certeza que deseja ativar o acesso de ${userToToggleStatus?.name}? O usuário poderá acessar o sistema novamente.`
          }
